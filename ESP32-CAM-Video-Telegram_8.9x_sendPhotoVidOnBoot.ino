...
String TIMEZONE = "WET-0WEST-1,M3.5.0/01,M10.5.0/02";
...
String chat_id = "0000";
#define BOTtoken "0000"  // your Bot Token (Get from Botfather)
...
void loop() {
  ...
  // camera turned ON by PIR, get photo and video ready
  if (powerOn_triggered) {
    powerOn_triggered = false;
    //  bot.sendMessage(CHANNEL_ID, "/clip", ""); delay (5000);
    bot.longPoll = 0;
    xTaskCreatePinnedToCore( the_camera_loop, "the_camera_loop", 10000, NULL, 1, &the_camera_loop_task, 1);
  }
  ...
  // video has been sent, wait 30sec and send commands for extra captures (until PIR output goes LOW)
  if (video_sent) {
    if (millis() > video_sent_time + 30000) {
      video_sent = false;

      String extracapture = "Still ON...\n\n";
      extracapture += "/photo: take a photo\n\n";
      extracapture += "/flash: toggle flash LED\n\n";
      extracapture += "/clip: short video clip\n\n";
      extracapture += "/start: start\n";

      bot.sendMessage(chat_id, extracapture, "Markdown");
    }
  }
...
void send_the_picture() {
  ...
  if (active_interupt) {
    ...
    //                  isMoreDataAvailable, getNextByte, nullptr, nullptr);
    String sent = bot.sendMultipartFormDataToTelegramWithCaption("sendPhoto", "photo", "img.jpg",
                  "image/jpeg", String(WiFi.RSSI()), chat_id, fb_length,
                  isMoreDataAvailable, getNextByte, nullptr, nullptr);
  }
